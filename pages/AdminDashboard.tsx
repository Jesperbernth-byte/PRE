...
import { Phone, Check, Clock, AlertCircle, Filter, Trash2, ExternalLink, LogOut, Sparkles, Edit } from 'lucide-react';
...
const LeadsView: React.FC<{ filter: 'nye' | 'alle' }> = ({ filter }) => {
  ...
  const filteredLeads = filter === 'nye' ? leads.filter(l => l.status === LeadStatus.NEW) : leads;
...
const AdminDashboardProtected: React.FC = () => {
  ...
  const isActive = (path: string) => {
    if (path === '/admin' || path === '/admin/nye') {
      return location.pathname === '/admin' || location.pathname === '/admin/nye' || location.pathname === '/admin/';
    }
  ...
  return (
    <div className="min-h-screen bg-slate-100">
        ...
        <div className="max-w-6xl mx-auto flex flex-col sm:flex-row justify-between items-center gap-4">
          ...
          <div className="flex items-center gap-4">
            <div className="flex bg-slate-800 p-1 rounded-xl">
              <button
                onClick={() => navigate('/admin/nye')}
                className={`px-6 py-2 rounded-lg font-bold transition-all ${isActive('/admin/nye') ? 'bg-orange-600 text-white' : 'text-slate-400'}`}
              >
                Nye Leads ({leads.filter(l => l.status === LeadStatus.NEW).length})
              </button>
              <button
                onClick={() => navigate('/admin/alle')}
                className={`px-6 py-2 rounded-lg font-bold transition-all ${isActive('/admin/alle') ? 'bg-orange-600 text-white' : 'text-slate-400'}`}
              >
                Alle
              </button>
              <button
              ...
              <button
                ...
                className={`px-6 py-2 rounded-lg font-bold transition-all flex items-center gap-2 ${isActive('/admin/rediger-indhold') ? 'bg-orange-600 text-white' : 'text-slate-400'}`}
              >
                <Edit size={16} />
                Rediger Indhold
              </button>
      ...
      <div className="max-w-6xl mx-auto p-4 sm:p-6 lg:p-10">
        <Routes>
          <Route index element={<LeadsView filter="nye" />} />
          <Route path="nye" element={<LeadsView filter="nye" />} />
          <Route path="alle" element={<LeadsView filter="alle" />} />
